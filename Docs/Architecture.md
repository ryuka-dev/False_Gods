...
| **FalseGods.RuntimeContracts** | The stable seam shared with optional adapters. Presentation contracts (`PresentationState`, `PresentationEvent`, `IEncounterPresentation`); outer capability ports (`IPlayerRoster`, `IMultiplayerSession`, `IEncounterChannel`, `IArenaLockdownPort`, `IRemoteNpcActivationPort`, `IEncounterReadyGate`); the arena realization seam (`IArenaAssetProvider`, `IArenaRealization`, with the `ArenaWorldPoint`/`ArenaRotation` carriers) — implemented by `UnityRuntime`, which cannot see `Application`, so these sit here for the same reason `IEncounterPresentation` does; transport-neutral carriers (`SessionPeerId`, `EncodedPayload`, `MessageDelivery`); the optional-integration seam (`IFalseGodsIntegration`, `IIntegrationRegistration`, and the single-slot `FalseGodsIntegrations` broker); `ILogger` | `FalseGods.Protocol` DTOs, UnityEngine, ST/LiteNetLib/Steamworks, any concrete implementation, any second piece of static state | Core |
| **FalseGods.Application** | Encounter & arena orchestration: `EncounterHost`, `ArenaLoadFlow`, ready-gate policy, teardown ordering, replication codecs over `IEncounterChannel`, and the **mappers** `Protocol DTO → PresentationState/PresentationEvent` and `domain state → PresentationState/PresentationEvent`. Ports whose consumer lives here **and whose implementation can reference Application**: `IEncounterReplication`, `IDamagePort`, `ISpawnPort`, `INavigationPort`, `IVanillaAssetProvider`, `ISceneLifecycleEvents` (the UnityRuntime-implemented `IArenaAssetProvider`/`IArenaRealization` live in RuntimeContracts — see that row) | UnityEngine, Harmony, ST/LiteNetLib/Steamworks, presentation implementations | Core, Protocol, RuntimeContracts |
...
| **RuntimeContracts** | `IPlayerRoster`, `IMultiplayerSession`, `IEncounterChannel`, `IArenaLockdownPort`, `IRemoteNpcActivationPort`, `IEncounterReadyGate`, `IEncounterPresentation`, `IArenaAssetProvider`, `IArenaRealization`, `ILogger` | Implemented by a module that cannot reference `Application`: *either* `Integration.Sulfur` / the optional ST adapter (session ports), *or* `UnityRuntime` (presentation and arena realization — its reference list is Core + RuntimeContracts + UnityEngine only). A port must live in an assembly its implementer can see. |
| **Application** | `IEncounterReplication`, `IDamagePort`, `ISpawnPort`, `INavigationPort`, `IVanillaAssetProvider`, `ISceneLifecycleEvents` | Consumed by the orchestration flows; implemented by `Integration.Sulfur`, which references `Application`. Nothing in Core calls them. |
