...
namespace FalseGods.Integration.Sulfur.Arena
{
    ...
    public sealed class SulfurArenaHijackPort : IArenaHijackPort
    {
        ...
        /// <summary>The one level this port declares to be the arena.</summary>
        private static readonly ArenaLevel ArenaLevel = new ArenaLevel(CaveEnvironment, FirstLevelIndex);
        ...
        public void LoadHijackedArena()
        {
            ...
            // Declare the level ours first, then ask for it. The declaration stands past this request on purpose:
            // in a session this same level is regenerated by paths we do not initiate, and each of those has to
            // build the arena too (LevelGenerationHijack). It is released by LeaveArenaMode, not by the load.
            LevelGenerationHijack.EnterArenaMode(ArenaLevel);
            try
            {
