...
{
    /// <summary>
    /// A throwaway, probe-local implementation of the real <see cref="IEncounterPresentation"/> seam: a minimal
    /// billboard boss (body, weak point, muzzle, health bar, ground telegraph) driven <b>only</b> by
    /// <see cref="Apply"/> (continuous state) and <see cref="Handle"/> (discrete cues).
    /// </summary>
    /// <remarks>
    /// This is the B1/B2 vertical slice seen from the renderer's side, and the vehicle for RiskList R15 (does a
    /// multi-part 2D boss face right, sort against the arena, and keep its hitboxes on its visible parts?) and
    /// R16/R27 (presentation is inert unless driven). The production renderer lives in <c>FalseGods.UnityRuntime</c>;
    /// this one lives in the probe so it stays deletable and so first light happens after a small step.
    ///
    /// <para>
    /// Facing follows SULFUR's own billboard model (Docs/Decompiled <c>NpcUpdateManager.UpdateFromPOV</c>): a sprite
    /// turns toward the camera <em>position</em>, not the camera's look direction — so rotating the view never spins
    /// the boss, only moving does. <see cref="FacingMode"/> selects between the three strategies (see
    /// <see cref="BossFacingMode"/>); the flat parts (body, weak point) live under one pivot that applies the chosen
    /// facing, while the health bar keeps its own always-face-the-camera billboard so it stays readable.
    /// </para>
    ///
    /// <para>
    /// It decides <b>nothing</b> authoritative — not damage, phase, death, target, or attack outcome. Where the boss
    /// is and what it is doing arrive through <see cref="Apply"/>; discrete one-shots through <see cref="Handle"/>.
    /// <see cref="Render"/> is a presentation-internal visual step (billboard facing, cue interpolation, flash decay)
    /// that reads only the camera transform and advances no boss state; calling none of the three leaves the stage
    /// frozen, which is the R16/R27 property under test.
    /// </para>
    /// </remarks>
    internal sealed class BossPresentationRenderer : IEncounterPresentation, IDisposable
    {
        ...
        public BossPresentationRenderer(ProbeReport report, Vector3 origin)
        {
            _report = report;
            _floorY = origin.y;
            _shader = ResolveShader(report, out _shaderName);

            _root = new GameObject("FalseGodsBossProbe");
            ...
            {
                report.Line("  !! 'Entities' layer not found; boss collision left on the Default layer.");
            }
            ...
            report.Value("boss renderer shader", $"{_shaderName} (supported={(_shader != null && _shader.isSupported)})");
            report.Value("boss renderer floor y", _floorY);
            report.Value("boss collision", $"capsule h={BodyHeight} r={BodyWidth * 0.45f:0.00} on layer "
                + $"'{(entitiesLayer >= 0 ? "Entities" : "Default")}' (kinematic Rigidbody)");
        }

        /// <summary>The sprite orientation strategy (see <see cref="BossFacingMode"/>). Settable live by the probe.</summary>
        ...
        /// <summary>The boss body's world collider — the probe raycasts against it to deal damage where you aim.</summary>
        ...
        public void Handle(IPresentationEvent presentationEvent)
        {
            switch (presentationEvent)
            {
                ...
                case AttackTelegraphStarted e:
                    ...
                    _report.Line($"  [cue] AttackTelegraphStarted {e.Kind} attack={e.Attack.Value} aim=({e.AimPoint.X:0.0},{e.AimPoint.Z:0.0})");
                    break;
                case AttackLanded e:
                    _telegraphActive = false;
                    HideTelegraph();
                    SpawnImpact(e.Kind, new Vector3(e.AimPoint.X, _floorY, e.AimPoint.Z));
                    _report.Line($"  [cue] AttackLanded {e.Kind} attack={e.Attack.Value}");
                    break;
                ...
                case PhaseTransition e:
                    ...
                    _report.Line($"  [cue] PhaseTransition -> phaseVisual={e.PhaseVisualId}");
                    break;
                case BossHit e:
                    _flashTimer = 0.12f;
        ...
        private void UpdateBodyColor()
        {
            ...
            {
                color = _hasState && _state.PhaseVisualId >= (int)FalseGods.Core.Bosses.BossPhase.Two
                    ? PhaseTwoColor
                    : PhaseOneColor;
            }
        ...
        /// </summary>
        private static Shader ResolveShader(ProbeReport report, out string chosenName)
        {
            string[] candidates =
            {
            ...
            report.Line("  !! No unlit colour shader resolved; boss will render with the fallback (magenta).");
